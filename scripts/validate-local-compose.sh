...
echo "Check 1/6: Local artifacts are available"
"$repo_root/scripts/sync-local-artifacts.sh"

echo "Check 2/6: Compose file renders"
compose_cmd config >/dev/null

echo "Check 3/6: Previous stack is cleared"
cleanup

echo "Check 4/6: Stack start is requested"
timeout 300s docker compose -f "$compose_file" --env-file "$compose_env" up -d artifact-server hex-core-service re-indicators-calculation-service >/tmp/re-indicators-local-up.log 2>&1 || {
  cat /tmp/re-indicators-local-up.log >&2
  exit 1
}

echo "Check 5/6: Core and calculation service are ready"
wait_for_http_code "http://127.0.0.1:8080/admin/ready" "200" "120"
wait_for_http_code "http://127.0.0.1:8081/health" "200" "120"

echo "Check 6/6: Compute request succeeds against local stack"
